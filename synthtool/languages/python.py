...
def configure_previous_major_version_branches() -> None:
    ...
    if list(Path(".").glob("google/**/version.py")):
        version_source_file = list(Path(".").glob("google/**/version.py"))[
            0
        ].read_text()
    else:
    ...
    VERSION_REGEX = (
        "(?:__)?version(?:__)?\s*=\s*[\"'](?P<major_version>\d)\.[\d\.]+[\"']"
    )
    major_version = int(
        re.search(VERSION_REGEX, version_source_file).group("major_version")
    )

    branch_release_config = """  # NOTE: this section is generated by synthtool.languages.python
