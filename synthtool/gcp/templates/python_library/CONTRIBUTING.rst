...
.. contents:: Here are some guidelines for hacking on Python Google Cloud Client libraries..
...
- Clone your fork of ``{{ metadata['repo']['repo'].split('/')[1] }}`` from your GitHub account to your local
  ...
  as ``hack-on-{{ metadata['repo']['repo'].split('/')[1] }}``.  E.g.::
   ...
   # Configure remotes such that you can pull changes from the {{ metadata['repo']['repo'].split('/')[1] }}
   # repository into your local repository.
   $ git remote add upstream git@github.com:GoogleCloudPlatform/{{ metadata['repo']['repo'].split('/')[1] }}.git
