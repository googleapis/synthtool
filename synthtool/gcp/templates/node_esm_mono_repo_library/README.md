...
{% else -%}
# [{{ metadata['repo']['name_pretty'] }}: {{ metadata['repo']['language']|language_pretty }} Client]({{ metadata['homepage'] }})
{%- endif %}
...
Samples are in the [`samples/`]({{  metadata['homepage'] }}/samples) directory. Each sample's `README.md` has instructions for running its sample.
